...
screen main_screen:
    frame:
        has vbox
        hbox:
            text "Date: [currDate.month], [currDate.day], [currDate.year] "
            python:
                formatedTime = currDate.strftime("%I:%M %p")
            text "Time: [formatedTime]"
            textbutton "Actions" action ui.callsinnewcontext("actions_screen_label")
            textbutton "Personal Info" action ui.callsinnewcontext("personal_info_screen_label")
            textbutton "Quit" action Quit()
        
label main_screen_label:
...
screen personal_info_screen:
    frame:
        ...
        text "Birth Date: [birthYear]"
        text "Gender: [gender]"
        text "Bio: [Bio]"
        hbox:
            textbutton "Return" action Return()
            textbutton "Skills" action ui.callsinnewcontext("skills_screen_label")

label personal_info_screen_label:
    call creating_bio
    call screen personal_info_screen
    return
...
screen actions_screen:
    frame:
        has vbox
        hbox:
            textbutton "Go to school. " action ui.callsinnewcontext("goToSchool")
            textbutton "Go home. " action ui.callsinnewcontext("goHome")
            textbutton "Return" action Return()
            
label goToSchool:
    "You decide to go to school."
    call school_screen_label
    return
label goHome:
    "You decide to go home."
    call home_screen_label
    return
            
label actions_screen_label:
...
screen skills_screen:
    frame:
        has vbox
        text "Fundamental Stats"
        text "Intelligence: [intelligence]"
        text "Strength: [strength]"
        text "Knowledge Skills"
        text "Knowledge of History: [knowledgeOfHistory]"
        text "Knowledge of Math: [knowledgeOfMath]"
        text "Knowledge of Science: [knowledgeOfScience]"
        textbutton "Return" action Return()
    
label skills_screen_label:
    call screen skills_screen
    return
    
screen academic_career_screen:
    frame:
        has vbox
...
screen school_screen:
    frame:
        has vbox
        textbutton "Go to class. " action ui.callsinnewcontext("class_screen_label")
        textbutton "Return" action Return()
        
screen class_screen:
    frame:
        has vbox
        if currDate >= datetime.datetime(currDate.year,currDate.month,currDate.day,hour = 8, minute = 30) and currDate <= datetime.datetime(currDate.year,currDate.month,currDate.day,hour = 15, minute = 30):
            textbutton "History" action ui.callsinnewcontext("historyClass")
        if currDate >= datetime.datetime(currDate.year, currDate.month, currDate.day, hour = 8, minute = 30) and currDate <= datetime.datetime(currDate.year, currDate.month, currDate.day, hour = 15, minute = 30):
            textbutton "Math" action ui.callsinnewcontext("mathClass")
        if currDate >= datetime.datetime(currDate.year, currDate.month, currDate.day, hour = 8, minute = 30) and currDate <= datetime.datetime(currDate.year, currDate.month, currDate.day, hour = 15, minute = 30):
            textbutton "Science" action ui.callsinnewcontext("scienceClass")
        textbutton "Return" action Return()
...
label class_screen_label:
    call screen class_screen
    return
        
label school_screen_label:
...
screen home_screen:
    frame:
        has vbox
        textbutton "Go to sleep. " action ui.callsinnewcontext("goToSleep")
        textbutton "Lift weights. " action ui.callsinnewcontext("liftWeights")
        textbutton "Return" action Return()
        
label goToSleep:
    "You decide to sleep"
    $ currDate = currDate + datetime.timedelta(hours = 8)
    return
    
label liftWeights:
    "You decide to lift weights"
    $ strength += renpy.random.randint(1,3)
    $ currDate = currDate + datetime.timedelta(minutes = 30)
    return
    


label home_screen_label:
